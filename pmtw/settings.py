...
class removalReason(object):
    ...
    def __init__(self, pmsubject, logreason, header, footer, removalOption, typeReply, typeStickied, typeLockComment,
                 typeAsSub, autoArchive, typeLockThread, logsub, logtitle, bantitle, getfrom, reasons):
...
class Settings(object):
    ...
    def __fetch_from_reddit(self):
        ...
        if settings['ver'] != settings_schema:
            raise RuntimeError(
                f"Settings Schema mismatch. PMTW requires {settings_schema}, wiki page is {settings['ver']}")
        else:
    ...
    def push_settings(self, reason='Toolbox Settings update from pmtw'):
        ...
        if len(wikipage_data) > max_wiki_size:
            raise OverflowError(
                f'settings data {len(wikipage_data) - max_wiki_size} bytes too big to insert')
    ...
    def get_domainTags(self):
        ...
        try:
            ...
            raise KeyError(
                "Section ['domainTags'] does not seem to exist in settings")
        return domainTags
    ...
    def get_removalReasons(self):
        ...
        try:
            ...
            removalReasons = removalReason(
                pmsubject=item['pmsubject'] if 'pmsubject' in item.keys(
                ) else None,
                logreason=item['logreason'] if 'logreason' in item.keys(
                ) else None,
                header=self.__decode_text(
                    item['header']) if 'header' in item.keys() else None,
                footer=self.__decode_text(
                    item['footer']) if 'footer' in item.keys() else None,
                removalOption=item['removalOption'] if 'removalOption' in item.keys(
                ) else None,
                typeReply=item['typeReply'] if 'typeReply' in item.keys(
                ) else None,
                typeStickied=item['typeStickied'] if 'typeStickied' in item.keys(
                ) else None,
                typeLockComment=item['typeLockComment'] if 'typeLockComment' in item.keys(
                ) else None,
                typeAsSub=item['typeAsSub'] if 'typeAsSub' in item.keys(
                ) else None,
                autoArchive=item['autoArchive'] if 'autoArchive' in item.keys(
                ) else None,
                typeLockThread=item['typeLockThread'] if 'typeLockThread' in item.keys(
                ) else None,
                logsub=item['logsub'] if 'logsub' in item.keys() else None,
                logtitle=item['logtitle'] if 'logtitle' in item.keys(
                ) else None,
                bantitle=item['bantitle'] if 'bantitle' in item.keys(
                ) else None,
            ...
            raise KeyError(
                "Section ['removalReasons'] does not seem to exist in settings")
        return removalReasons
    ...
    def get_reasons(self):
        ...
        try:
            for item in self.settingsJSON['removalReasons']['reasons']:
                reasons.append(
                    reason(
                        ...
                        flairText=item['flairText'] if 'flairText' in item.keys(
                        ) else None,
                        flairCSS=item['flairCSS'] if 'flairCSS' in item.keys(
                        ) else None,
                        removePosts=item['removePosts'] if 'removePosts' in item.keys(
                        ) else None,
                        removeComments=item['removeComments'] if 'removeComments' in item.keys(
                        ) else None,
            ...
            raise KeyError(
                "Section ['removalReasons']['reasons'] does not seem to exist in settings")
        return reasons
    ...
    def get_modMacros(self):
        ...
        try:
            for item in self.settingsJSON['modMacros']:
                modMacros.append(
                    modMacro(
                        text=self.__decode_text(
                            item['text']) if 'text' in item.keys() else None,
                        title=self.__decode_text(
                            item['title']) if 'title' in item.keys() else None,
                        distinguish=item['distinguish'] if 'distinguish' in item.keys(
                        ) else None,
                        ban=item['ban'] if 'ban' in item.keys() else None,
                        mute=item['mute'] if 'mute' in item.keys() else None,
                        remove=item['remove'] if 'remove' in item.keys(
                        ) else None,
                        approve=item['approve'] if 'approve' in item.keys(
                        ) else None,
                        lockthread=item['lockthread'] if 'lockthread' in item.keys(
                        ) else None,
                        lockreply=item['lockreply'] if 'lockreply' in item.keys(
                        ) else None,
                        sticky=item['sticky'] if 'sticky' in item.keys(
                        ) else None,
                        archivemodmail=item['archivemodmail'] if 'archivemodmail' in item.keys(
                        ) else None,
                        highlightmodmail=item['highlightmodmail'] if 'highlightmodmail' in item.keys(
                        ) else None,
                        contextpost=item['contextpost'] if 'contextpost' in item.keys(
                        ) else None,
                        contextcomment=item['contextcomment'] if 'contextcomment' in item.keys(
                        ) else None,
                        contextmodmail=item['contextmodmail'] if 'contextmodmail' in item.keys(
                        ) else None
                    )
                )
        except KeyError:
            raise KeyError(
                "Section ['modMacros'] does not seem to exist in settings")
        return modMacros
    ...
    def get_usernoteColors(self):
        ...
        try:
            ...
            raise KeyError(
                "Section ['usernoteColors'] does not seem to exist in settings")
    ...
    def get_banMacros(self):
        ...
        try:
            for item in self.settingsJSON['']:
                banMacros.append(
                    banMacro(
                        banNote=item['banNote'] if 'banNote' in item.keys(
                        ) else None,
                        banMessage=item['banMessage'] if 'banMessage' in item.keys(
                        ) else None
                    )
                )
        except KeyError:
            raise KeyError(
                "Section ['banMacros'] does not seem to exist in settings")
        return banMacros
