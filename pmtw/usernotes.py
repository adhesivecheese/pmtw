...
class Usernotes(object):
    ...
    def __fetch_from_reddit(self):
        ...
        try:
            ...
            raise RuntimeError("Usernotes page does not exist") from e
    ...
    def warnings_types(self):
        """
        Return warning types of notes on userpage. 

        Use warnings_types(local="False") to fetch notes from Reddit before listing warnings
        """
    ...
    def delete_note(self, username, timestamp):
        """
        Delete a note on a user. 
        
        Requires a username, and the timestamp of the note to delete.
        By default, immediately updates wiki page. You can override immediate update by passing delete_note(note, batch="True");
        if so, you will need to manually sync notes with push_usernotes() 
        """
    ...
    def purge_user_notes(self, username):
        """
        Delete all notes on a user. 
        
        Requires a username. By default, immediately updates the wiki page. 
